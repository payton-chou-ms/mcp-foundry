...
async def fetch_finetuning_status(job_id: str) -> str:
    ...
    """
    if not AGENT_INITIALIZED:
        return json.dumps({"error": "Azure AI Agent service is not initialized. Check environment variables."})
    ...
    try:
        ...
        logger.error(f"Error fetching fine-tuning job status: {str(e)}")
